...
export const auditApi = {
  ...
  getAll: (params?: { module?: string; action?: string; startDate?: string; endDate?: string }) => {
    ...
    return apiClient.request(`${ADMIN_BASE}/audit${query ? `?${query}` : ''}`);
  },

  // Fetch a single log entry by ID
  getById: (id: string) => apiClient.request(`${ADMIN_BASE}/audit/${id}`),
  ...
  create: (data: {
    ...
    entityId?: string;
    details?: string;
  }) => apiClient.request(`${ADMIN_BASE}/audit`, { method: 'POST', body: JSON.stringify(data) }),
};
